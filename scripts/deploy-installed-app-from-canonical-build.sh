...
CANONICAL_RUNTIME_DIR="${TASKNERVE_CANONICAL_RUNTIME_DIR:-${TASKNERVE_LIVE_EXTRACT_DIR:-$REPO_ROOT/target/codex-tasknerve-app-live-extract}}"
...
if [[ ! -d "$CANONICAL_RUNTIME_DIR" ]]; then
  echo "missing canonical generated runtime artifact: $CANONICAL_RUNTIME_DIR" >&2
  echo "expected artifact at target/codex-tasknerve-app-live-extract (alias: target/codex-tasknerve-app-src)" >&2
  echo "implementation source-of-truth is codex-native/src; do not hand-edit bundle artifacts" >&2
  exit 1
fi
...
LIVE_PACKAGE_JSON="$CANONICAL_RUNTIME_DIR/package.json"
...
node "$ASAR_BIN" pack "$CANONICAL_RUNTIME_DIR" "$TMP_ASAR"
...
echo "deployed canonical generated runtime artifact to installed app"
echo "app: $APP_PATH"
