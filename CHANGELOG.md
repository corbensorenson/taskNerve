...
- Added auto-seeded root project contracts: `project_goals.md` for locked outcomes and `project_manifest.md` for locked technical implementation rules. TaskNerve now ensures both files on project add/use and GUI selection, includes them in advisor prompts/snapshots/run reports, and points worker continuation prompts back at them as the durable project contract.
