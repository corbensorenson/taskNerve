...
export function renderMarkdownBullets(items: string[] = [], fallback: string): string {
  if (items.length === 0) {
    return `- ${fallback}`;
  }
...
export function renderProjectGoalsTemplate(summary: ProjectContractSummary): string {
...
export function renderProjectManifestTemplate(summary: ProjectContractSummary): string {
  const generatedAt = summary.generatedAt ?? nowIsoUtc();
