import { parseTimestampUtc, timestampDisplayFromIso, } from "./timestamps.js";
const MAX_SCAN_OBJECTS = 8000;
const MAX_TEXT_SCAN_OBJECTS = 48;
const KNOWN_TURN_ARRAY_PATHS = [
    ["conversation", "turns"],
    ["thread", "turns"],
    ["turns"],
...
const threadExtractionCache = new WeakMap();
const turnArrayExtractionCache = new WeakMap();
...
function looksLikeTurnRecord(value) {
    const entry = asRecord(value);
    if (!entry || !("id" in entry)) {
        return false;
    }
    return (Array.isArray(entry.input_items) ||
...
function normalizeTurnId(value, fallbackIndex) {
    const text = String(value ?? "").trim();
    return text || `turn-${fallbackIndex + 1}`;
}
function turnTimestamp(turn) {
    return (parseTimestampUtc(turn.created_at) ||
...
function normalizeTurnArray(value) {
    if (Array.isArray(value)) {
        return value.filter((entry) => looksLikeTurnRecord(entry));
    }
    ...
    return Object.values(record)
        ...
        .filter((entry) => looksLikeTurnRecord(entry));
}
function dedupeSortTurns(turns) {
...
function extractTurnsViaKnownPaths(thread) {
...
function extractTurnsByScan(thread) {
    const queue = [thread];
    const seen = new Set();
    const turns = [];
    let scanned = 0;
    ...
    if (turns.length === 0 && Array.isArray(thread)) {
        return thread.filter((entry) => looksLikeTurnRecord(entry));
    }
...
function flattenText(value) {
    ...
    const pieces = [];
    let scanned = 0;
    for (let queueIndex = 0; queueIndex < queue.length && scanned < MAX_TEXT_SCAN_OBJECTS; queueIndex += 1) {
        const current = queue[queueIndex];
        scanned += 1;
...
function inferRole(defaultRole, item) {
...
function summarizeItem(item, fallbackPrefix) {
    const type = itemType(item);
    if (item && typeof item === "object" && type && type !== "message") {
        const record = item;
        const name = stringFromUnknown(record.name);
        const detail = flattenText(record.detail ?? record.arguments ?? record.result ?? record.output ?? record.input);
...
function itemTimestamp(item, fallback) {
    ...
    const record = item;
    return (parseTimestampUtc(record.created_at) ||
...
function turnEntries(turn, index) {
...
function hydrateMissingTimestamps(entries, generatedAtUtc) {
    let hasMissingTimestampData = false;
    for (const entry of entries) {
        if (!parseTimestampUtc(entry.created_at_utc) ||
...
const CACHEABLE_TURN_ARRAY_PATHS = [
    ["conversation", "turns"],
    ["thread", "turns"],
    ["turns"],
    ["messages"],
];
function cacheableTurnArray(thread) {
...
function fastTurnMarker(value) {
    const record = asRecord(value);
    if (!record) {
        return String(value ?? "");
    }
    const id = String(record.id ?? "");
    const timestamp = String(record.updated_at ??
        record.updatedAt ??
        record.timestamp ??
        record.created_at ??
        record.createdAt ??
        "");
    const inputCount = asArray(record.input_items).length +
...
function entriesCacheMarker(thread, generatedAtUtc) {
...
export function extractThreadDisplayEntries(thread, generatedAtUtc = new Date().toISOString()) {
