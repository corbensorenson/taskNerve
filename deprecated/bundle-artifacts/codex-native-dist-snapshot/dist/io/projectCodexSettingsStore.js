...
const projectCodexSettingsCache = new Map();
const projectCodexSettingsInflight = new Map();
function normalizeGitOriginUrl(gitOriginUrl) {
    const text = typeof gitOriginUrl === "string" ? gitOriginUrl.trim() : "";
    return text || null;
}
function settingsCacheKey(filePath, gitOriginUrl) {
    return `${filePath}${CACHE_KEY_SEPARATOR}${gitOriginUrl || ""}`;
}
function clearProjectCodexSettingsCacheForPath(filePath) {
...
function findProjectCodexSettingsCacheForPath(filePath) {
    const prefix = `${filePath}${CACHE_KEY_SEPARATOR}`;
...
export async function writeProjectCodexSettings(repoRoot, settings) {
    ...
    clearProjectCodexSettingsCacheForPath(filePath);
    projectCodexSettingsCache.set(settingsCacheKey(filePath, normalizeGitOriginUrl(normalized.git_origin_url)), {
        mtimeMs: nextMtimeMs,
        value: normalized,
        raw: nextRaw,
    });
...
//# sourceMappingURL=projectCodexSettingsStore.js.map